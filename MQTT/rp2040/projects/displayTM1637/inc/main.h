...
/**
 * @brief 긴급 종료 함수 (예상치 못한 오류 시)
 *
 * 시스템 상태를 로깅하고 정리한 후 재부팅을 대기합니다.
 *
 * @param reason 종료 사유
 * @param error_code 오류 코드
 */
inline void emergency_shutdown(const char *reason, int error_code)
{
    printf("\n[긴급] ===== 시스템 긴급 종료 =====\n");
    printf("[긴급] 사유: %s (Code: %d)\n", reason, error_code);
    printf("[긴급] 리소스 정리 중...\n");

    cleanup_resources();

    printf("[긴급] 모든 디스플레이 OFF\n");
    for (int i = 0; i < NUM_DISPLAYS; i++)
    {
        if (displays[i] != nullptr)
        {
    ...
    printf("[긴급] WDT 타임아웃 대기 중 (재부팅 예정)...\n");
    // WDT가 타임아웃될 때까지 무한 대기 (30초 후 자동 재부팅)
    while (true)
    {
        sleep_ms(1000);
    }
 ...
 * @brief 시스템 건강 상태 체크
 *
 * 주기적으로 시스템 상태를 확인하고 WDT를 갱신합니다.
 *
 * @return true 시스템 정상
 * @return false 시스템 이상 감지
 */
inline bool system_health_check()
{
    // WDT 타임아웃 방지 (주기적으로 WDT를 "먹여줌")
    watchdog_update();
    ...
    for (int i = 0; i < NUM_DISPLAYS; i++)
    {
        if (displays[i] == nullptr)
        {
            printf("[경고] 디스플레이 %d가 nullptr입니다!\n", i);
            // 심각한 오류이지만 계속 진행
        }
    }

    return true; // 현재는 항상 true 반환 (추후 더 정교한 체크 가능)
}
